...
double get_angle(p3 a, p3 b) {
    ...
    return acos(max((double)-1.0, min((double)1.0, costheta)));}
double small_angle(p3 v, p3 w) {return acos(min(fabs(v | w) / abs(v) / abs(w), (double)1.0));}
struct plane {// n is the perpendicular normal vector to the plane
    p3 n; double d; // (n | p) = d
...
struct coords {
    ...
    p3 pos3d(PT p){return o + dx * p.x + dy * p.y;}};
struct line3d {
